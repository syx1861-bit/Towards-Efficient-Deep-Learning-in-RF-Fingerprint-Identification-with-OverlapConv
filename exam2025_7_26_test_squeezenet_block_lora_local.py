...
from model_7_8_2.mobilenet import mobilenet_v1, mobilenet_v1_soft, mobilenet_v1_hard
from model_7_8_2.squeezenet import squeezenet, squeezenet_soft
from model_7_8_2.ghostnet import ghost_net, ghost_net_soft
from model_7_8_2.simplecnn2 import MobileNetV2CNN_spec_hard, GhostCNN_spec_hard, SqueezeCNN_spec_hard, \
    ShuffleNetV2CNN_spec_hard, LMSC_spec_hard
from model_7_8_2.fasternet_shorter_hard import FasterNet_shorter_hard
from model_7_8_2.fasternet_shorter_hard_add1 import FasterNet_shorter_hard_add1
from model_7_8_2.fasternet_shorter_hard_add2 import FasterNet_shorter_hard_add2
from model_7_8_2.mobilenetv4_short_hard import mobilenetv4_conv_small_short_hard
from model_7_8_2.mobilenetv4_short_hard_add2 import mobilenetv4_conv_small_short_hard_add2
from model_7_8_2.simplecnn_raw import SimpleCNN_hard, SimpleCNN_hard_8
from model_7_8_2.LMSC import my_resnet_hard
from model_7_8_2.deep_learning_models import ClassificationNet_hard3, ClassificationNet_hard
from model_7_8_2_efficientnet_lora_school.efficientv2_shorter3_pgroup_overlap import efficientnet_b0_shorter2_pgroup_overlap
from model_7_8_2_efficientnet_lora_school.efficientv2_shorter3_pgroup_overlap_add2 import efficientnet_b0_shorter2_pgroup_overlap_add2
from model_7_8_2_squeezenet_lora_school.squeezenet_short_hard import squeezenet_hard
from model_7_8_2_squeezenet_lora_school.squeezenet_short_hard_2 import squeezenet_hard_2
from model_7_8_2_squeezenet_lora_school.squeezenet_short_hard_3 import squeezenet_hard_3
from model_7_8_2_squeezenet_lora_school.squeezenet_short_hard_4 import squeezenet_hard_4
MAX_EPOCHS = 350
...
def train_hard_model(seed_dataset, model, train_loader, val_loader, criterion, optimizer, ckpt_dir, model_type="Hard",
                     oc=None):
    """Hard模型的训练函数 - 统一优化方案"""
    ...
    for epoch in range(1, MAX_EPOCHS_HARD + 1):
        ...
        #                os.path.join(ckpt_dir, f"{model_name}_best_hard.pth"))
        #     print(
        ...
        if val_acc > best_val_acc:
            ...
            best_model_state = copy.deepcopy(model.state_dict())
            torch.save(best_model_state,
                       os.path.join(ckpt_dir, f"{model_name}_best_hard_seed{seed_dataset}.pth"))
            print(
                f"New best model saved at epoch {epoch} with val_loss={best_val_loss:.4f}, val_acc={best_val_acc:.4f}")
...
def train_raw_model(model, train_loader, val_loader, criterion, optimizer, model_type="raw", ckpt_dir=None):
    """Hard模型的训练函数 - 统一优化方案"""
    best_val_loss = float('inf')
    best_val_acc = 0.0
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': []}
...
def train_soft_model(model, train_loader, val_loader, criterion, optimizer, device, ckpt_dir):
    """Soft模型的训练函数 - 统一优化方案"""
        ...
        if epoch > (MAX_EPOCHS - 50):
            if val_loss < best_val_loss_last50:
                ...
                torch.save(best_model_state_last50,
                           os.path.join(ckpt_dir, f"{model_name}_best_last50.pth"))
                print(
...
if __name__ == "__main__":  # 0.,  0.,2  03  045 05 06 65  07 08
    model_summary_list = [['squeezenet']]
    # model_summary_list = [ ['faster_hard']]
    for model_sub in model_summary_list:
        # overlap_list = [0.7,0.725,0.75,0.7855187267065048,0.8,0.825,0.85,0.875,0.885,0.8967,0.9,0.925,0.95,0.975,1.0,1.025,1.05,1.075,1.1,1.15,1.2,1.25,1.3,1.35,1.4]
        # overlap_list =[7855187267065048,0.8,0.825,0.85,0.875,0.885,0.8967,0.9,0.925,0.95,0.975,1.0,1.025,1.05,1.075,1.1,1.15,1.2,1.25,1.3,1.35,1.4]
        overlap_list =[0.22]
        # overlap_list = [0.725,0.4,0.65,0.75,0.85,0.95,1.05,1.15,1.25,1.35,1.4]
        #overlap_list = [0.61]
        #overlap_list = [1.25]
        for overlap_rate in overlap_list:
            global_seed = 2025
            ...
            seed_list = [42]
            # seed_list_dataset = [42, 43, 44, 45, 46]
            ...
            #test_snr_list = [(25, 25),(30,30),(35,35)]
            test_snr_list = [(30,30),]
            # model_list = ['mobilenet', 'squeezenet',ghostnet,'shufflenet','faster_hard','LMSC_hard'
            # 'mobilenetv4_short_hard','simplecnn_hard','faster_hard_add1','faster_hard_add2','mobilenetv4_short_hard_add2']
            model_list = model_sub
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            print(f"Using device: {device}")

            # 确保结果目录存在
            # os.makedirs("compare_results8_29", exist_ok=True)
            os.makedirs("compare_results_12_2_squeezenet_block_hard_lora_local", exist_ok=True)
            ...
            for seed_dataset in seed_list_dataset:
                ...
                for model_name in model_list:
                    ...
                    # soft_csv_path = f"compare_results7_17/{model_name}_soft.csv"
                    # hard_csv_path = f"results7_8/structure_{model_name}_hard.csv"
                    hard_csv_path = f"compare_results_12_2_squeezenet_block_hard_lora_local/{model_name}_hard.csv"
                    os.makedirs(os.path.dirname(hard_csv_path), exist_ok=True)

                    # 初始化CSV文件
                    for csv_path in [hard_csv_path]:
                    ...
                    for seed in seed_list:
                        ...
                        if model_name == 'mobilenet':
                            from model_7_8_2.mobilenet import SegmentedSoftGroupConv
                            # soft_model = mobilenet_v1_soft().to(device)

                        else:
                            from model_7_8_2.squeezenet import SegmentedSoftGroupConv
                        ...
                        #     soft_model, train_loader, val_loader, criterion, optimizer_soft, device, 'compare_results7_22'
                        # )
                        #
                        ...
                        if model_name == 'mobilenet':
                            # hard_model = mobilenet_v1_hard().to(device)
                            hard_model = MobileNetV2CNN_spec_hard(overlaprate=1.1).to(device)
                            print(hard_model)
                        elif model_name == "squeezenet":
                            hard_model = SqueezeCNN_spec_hard(overlaprate=0.95).to(device)
                        elif model_name == 'ghostnet':
                            hard_model = GhostCNN_spec_hard(overlaprate=1.4).to(device)
                        elif model_name == 'shufflenet':
                            hard_model = ShuffleNetV2CNN_spec_hard(overlaprate=1.0).to(device)
                        elif model_name == 'faster_hard':
                            hard_model = FasterNet_shorter_hard(overlaprate=overlap_rate).to(device)
                        elif model_name == 'mobilenetv4_short_hard':
                            hard_model = mobilenetv4_conv_small_short_hard(overlaprate=overlap_rate).to(device)
                        elif model_name == 'simplecnn_hard':
                            hard_model = SimpleCNN_hard(overlaprate=overlap_rate).to(device)
                            print(hard_model)
                        elif model_name == 'simplecnn_hard_8':
                            hard_model = SimpleCNN_hard_8(overlaprate=overlap_rate).to(device)
                        elif model_name == 'LMSC_hard':
                            hard_model = LMSC_spec_hard(overlaprate=0.5).to(device)
                        elif model_name == 'faster_hard_add1':
                            hard_model = FasterNet_shorter_hard_add1(overlaprate=0.60).to(device)
                        elif model_name == 'faster_hard_add2':
                            hard_model = FasterNet_shorter_hard_add2(overlaprate=overlap_rate).to(device)
                        elif model_name == 'mobilenetv4_short_hard_add2':
                            hard_model = mobilenetv4_conv_small_short_hard_add2(overlaprate=0.781).to(device)
                        elif model_name == 'lmsc_hard':
                            hard_model = my_resnet_hard(overlaprate=overlap_rate).to(device)
                        elif model_name == 'extractor_short_hard3':
                            hard_model = ClassificationNet_hard3(overlaprate=overlap_rate).to(device)
                        elif model_name=='efficientnet_shorter2_hard':
                            hard_model=efficientnet_b0_shorter2_pgroup_overlap(overlaprate=overlap_rate).to(device)
                        elif model_name=='efficientnet_shorter2_hard_add2':
                            hard_model = efficientnet_b0_shorter2_pgroup_overlap_add2(overlaprate=overlap_rate).to(device)
                        elif model_name=='squeezenet_raw_short_hard':
                            hard_model=squeezenet_hard(overlap_rate=overlap_rate).to(device)
                        elif model_name=='squeezenet_raw_short_hard2':
                            hard_model =squeezenet_hard_2(overlap_rate=overlap_rate).to(device)
                        elif model_name=='squeezenet_raw_short_hard3':
                            hard_model =squeezenet_hard_3(overlap_rate=overlap_rate).to(device)
                        elif model_name == 'squeezenet_raw_short_hard4':
                            hard_model = squeezenet_hard_4(overlap_rate=overlap_rate).to(device)
                        # elif model_name=='mobilenetv4_short_hard':
                        #     hard_model=mobilenetv4_conv_small_short_hard(overlaprate = 0.70).to(device)
                        else:
                            print("有问题")
                        ...
                        # 使用新的训练函数
                        hard_model, hard_history = train_hard_model(
                            seed_dataset, hard_model, train_loader, val_loader, criterion_hard, optimizer_hard,
                            model_type="Hard", ckpt_dir='compare_results_12_2_squeezenet_block_hard_lora_local'
                        )
                        for test_snr in test_snr_list:
                            # 评估外部测试集（不加噪声）
                            ext_test_acc = run_test(hard_model, TEST_FILE, DEV_RANGE, PKT_RANGE_test, test_snr=test_snr,
                                                    model_name=f"hard_model_{model_name}_hard_seed_{seed_dataset}")

                            # 保存结果
                            save_results_incremental(hard_csv_path, [[
                                model_name, seed_dataset, 4, overlap_rate,
